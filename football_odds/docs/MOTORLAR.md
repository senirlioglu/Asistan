...
|---|---|---|---|
| ★ · Günün raporu | bir gün | `daily.build_report`: günün maçları on ana hedef için sırayla `start_day_scan` ile taranır (tek ortak havuz, `SCAN_WORKERS` iş parçacığı), sonra `compile_report`: hedef başına motor deseni bulunan ya da katman toplamı fiyattan ayrışan maçlar "daha çok / daha az / çelişkili" diye ayrılır (yön: desenlerin işareti, desen yoksa toplam farkın işareti); öne çıkanlar = desen bulunanlar, araştırma önceliğine göre; defter notlarından yalnızca sonuç söyleyenler (`RESULT_NOTES`) ve en çok not toplayan maçlar. Sabahki günlük iş önce bugünün, sonra yarının raporunu üretir; saatlik iş yalnızca boşluk doldurur (bugünkü yoksa/12 saatten eskiyse, yarınki yoksa); `results/lab_daily/<gün>.json` | `POST/GET /api/lab/gunluk?date=` |
...
| 2 · Bu sonuca uyan maç bul | bir hedef | günün her maçında `target.analyse(light=True)` (katmanlar) **+ pattern motorları o hedef için** (`lab.scan(outcomes=(hedef,), light=True)`: form deseni üç seviye, iki kulüp için; iki takım birlikte kaskadı bir kez) **+ o maçta uyanan defter notları** (`NOTE_TARGETS`: hedefle ilgili olanlar işaretli). Arka planda, 4 maç paralel; saatlik durum yenilemesi işi düşürmez (`stale`). Sayfada iki sekme: Patternler (Nerede sütunu: takım / rakip / benzer / ikizler + bulunan desenler) ve Defter notları. Sıralama önce desen bulunanlar, sonra "araştırma önceliği" — bahis skoru değil | `POST/GET /api/lab/tara?date=&target=` |
...
/api/lab/tara              mod 2: günün maçlarını bir hedef için tara (POST başlatır, GET izler; biten tarama results/lab_scans/ altında, deploy'da kaybolmaz)
/api/lab/gunluk            günün raporu: on hedefin taraması derlenmiş (POST üretir, GET okur; saatlik iş kendisi üretir)
