...
if bt:
    verdict = ("adjusted model ≥ market out-of-sample" if bt.get("backtest_ok") else
               "historical similarity did NOT improve on the market out-of-sample")
...
st.dataframe(show.style.format({c: "{:.1f}" for c in num_cols if "ODDS" not in c} | {c: "{:.2f}" for c in num_cols if "ODDS" in c or c == "AVG GOALS"})
             ...
             use_container_width=True, hide_index=True, height=min(600, 60 + 35 * len(show)))
...
if scopes:
    ...
    o2.dataframe(pd.DataFrame(rows), hide_index=True, use_container_width=True)
tol = det.get("tolerance", {})
if tol:
    st.markdown("**Model A — tolerance matching (matches within ±tol on all three outcomes)**")
    st.dataframe(pd.DataFrame({mode: {f"±{float(k) * 100:.0f}%": v for k, v in lv.items()} for mode, lv in tol.items()}).T, use_container_width=True)
...
if not analogues.empty:
    ...
    st.dataframe(show_an.style.format({"Odds H": "{:.2f}", "Odds D": "{:.2f}", "Odds A": "{:.2f}", "Prob H %": "{:.1f}", "Prob D %": "{:.1f}",
                 ...
                 hide_index=True, use_container_width=True, height=min(700, 60 + 35 * len(show_an)))
