...
a tie between clubs of two leagues (European cups, domestic cups) is placed under `CUP` with each club's
standing taken from its own league.

**Günün raporu** (`src/patterns/daily.py`) is the lab's front page: the day's matches through every
main target (1, X, 2, 2,5 üst, KG var, İY/MS 1/2 and 2/1, İY 1/X/2), scanned in turn on the lab's one
