...
3. re-reads the fixture list every FO_FIXTURES_EVERY_MIN minutes and analyses whatever was published
   since the morning run,
4. serves the Streamlit dashboard on $PORT.
...
Environment:
    ...
    FO_FIXTURES_EVERY_MIN  minutes between intraday fixture re-reads (default 60; 0 disables). Football-Data
                    adds a fixture to fixtures.csv only once its odds are in, so the morning run misses the
                    matches published later that day
...
def scheduler_loop(settings, hhmm: str, days: int, scorecard_hhmm: str, fixtures_every_min: int = 60) -> None:
    ...
    # Football-Data publishes a fixture only once its odds are in, so the list grows through the day: the
    # morning run can legitimately see 2 matches where the evening holds 20. Re-read it on an interval.
    # the first one soon after boot: a deploy in the evening should not wait a whole interval for the
    # matches the morning run could not see
    next_fixtures = time.monotonic() + 300 if fixtures_every_min > 0 else None
    while True:
        waits = [(seconds_until(t), name, fn) for t, name, fn in jobs]
        if next_fixtures is not None:
            waits.append((max(next_fixtures - time.monotonic(), 0), "fixtures", lambda: run_fixture_refresh(settings, days=days)))
        wait, name, fn = min(waits, key=lambda x: x[0])
        log.info("next job '%s' in %.0f min", name, wait / 60)
        time.sleep(wait)
        fn()
        if name == "fixtures":
            next_fixtures = time.monotonic() + fixtures_every_min * 60
...
def main() -> int:
    ...
    fixtures_every = int(os.environ.get("FO_FIXTURES_EVERY_MIN", "60"))
    port = os.environ.get("PORT", "8501")

    threading.Thread(target=scheduler_loop, args=(settings, hhmm, days, scorecard_hhmm, fixtures_every), name="fo-scheduler", daemon=True).start()
